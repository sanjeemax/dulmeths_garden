...
*/
 // helper: derive pageId/postId from "<pageId>_<postId>" URLs if needed
function extractIds(post) {
    if (post.pageId && post.postId) {
        return { pageId: post.pageId, postId: post.postId };
    }
    const m = (post.facebookUrl || "").match(/facebook\.com\/(\d+)_(\d+)/);
    return m ? { pageId: m[1], postId: m[2] } : {};
}
